...
Script Purpose:
	...
	Actions Performed:
		...
		- Records a row in dbo.load_audit for every table loaded (success or
		  failure), grouped by a run_id so one EXEC's audit trail can be
		  queried together.
...
BEGIN
    ...
    BEGIN TRY
		...
		SELECT
			...
			CASE
				...
				ELSE bdate
			END AS bdate, -- Set out-of-range birthdates to NULL
			CASE
	...
	BEGIN CATCH
		...
		VALUES (@run_id, 'silver', 'silver.load_silver', @current_table, @start_time, GETDATE(), DATEDIFF(MILLISECOND, @start_time, GETDATE()), NULL, 'FAILED', ERROR_MESSAGE());
